        ...
        try {
            ...
            foreach ($exchangeRates['quotes'] as $key => $rate) {
                ...
                $this->exchangeRateRepository->updateOrCreate(
                    ['target_currency' => $targetCurrencyId], // Use currency ID instead of code
                    ['rate' => $rate]
                );
            }
            ...
            session()->flash('error', 'Failed to update exchange rates: ' . $e->getMessage());
        }
