...
@php
    // Query categories with product counts using the Category model.
    // This returns a collection where each category has a 'products_count' attribute.
    $categoriesForCounts = \Webkul\Category\Models\Category::withCount('products')->get();
    // Create an associative array: [ category_id => products_count, ... ]
    $counts = $categoriesForCounts->pluck('products_count', 'id');
@endphp
<script>
    window.parentCategoryCounts = {!! json_encode($counts) !!};
</script>
          ...
          <ul class="p-5 bg-gray-50">
            <li v-for="child in activeParent.children"
              ...
              <div class="flex items-center justify-between">
                <template v-if="child.products_count > 0">
                  <a :href="child.url"
              ...
              <ul v-if="expandedChildId === child.id" class="pl-4 mt-2">
                <li v-for="grandchild in child.children"
                    :key="grandchild.id"
                    class="py-1">
                  <template v-if="grandchild.products_count > 0">
  ...
  app.component('v-desktop-category', {
    ...
    methods: {
      getParentCategories() {
        this.$axios.get("{{ route('shop.api.categories.tree') }}")
          .then(response => {
            ...
            // Merge product counts from PHP global variable
      ...
      setActiveParent(parent) {
        ...
        this.activeChild = (parent.children && parent.children.length) ? parent.children[0] : null;
