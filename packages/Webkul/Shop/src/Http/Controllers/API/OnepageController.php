...
class OnepageController extends APIController
{
    ...
    public function storeOrder()
    {
        ...
        // Get the raw currency from the cart or fallback
        $rawCurrency = isset($cart->cart_currency_code) ? $cart->cart_currency_code : config('myfatoora.country_iso');
            
        // Map the raw currency to allowed vcCodes for MyFatoorah
        $currencyMapping = [
            'KWD' => 'KWT', 
            'AED' => 'ARE',
            'SAR' => 'SAU',
        ...
        $countryIso = isset($currencyMapping[$rawCurrency]) ? $currencyMapping[$rawCurrency] : $rawCurrency;

        // Deactivate the cart and flash order id to session
        Cart::deActivateCart();
        session()->flash('order_id', $order->id);

        // Now check for payment redirection
        if ($redirectUrl = Payment::getRedirectUrl($cart)) {
           // Append the order id and country iso as query parameters
           $redirectUrl .= '?order_id=' . $order->id . '&country_iso=' . $countryIso;
